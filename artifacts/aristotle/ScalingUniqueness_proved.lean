...
This project request had uuid: 1c3414f4-fd21-47b5-99a1-2ab27892df92
...
theorem scaling_uniqueness
    ...
    False := by
  have := hkΦ_eq x₀;
  rw [ show ops.laplacian ( fun y => k * Φ y ) x₀ = k * ops.laplacian Φ x₀ + ops.laplacian ( fun _ => 0 ) x₀ from ?_, show ops.gradNorm ( fun y => k * Φ y ) x₀ = |k| * ops.gradNorm Φ x₀ from ?_ ] at this;
  · have := ops.laplacian_linear ( fun _ => 0 ) ( fun _ => 0 ) ( -1 ) ; simp_all +decide [ ne_of_gt ] ;
    rw [ max_eq_left ( by positivity ) ] at *;
    rw [ abs_of_pos ( zero_lt_one.trans hk ) ] at *;
    rw [ Real.mul_rpow ( by positivity ) ( by positivity ) ] at *;
    have := hΦ_eq x₀; rw [ max_eq_left ( by positivity ) ] at this; nlinarith [ show 0 < ctx.c x₀ * Φ x₀ ^ ctx.p by positivity, show k ^ ctx.p > k by exact lt_of_le_of_lt ( by norm_num ) ( Real.rpow_lt_rpow_of_exponent_lt hk ( show ctx.p > 1 by linarith [ ctx.p_gt_one ] ) ) ] ;
